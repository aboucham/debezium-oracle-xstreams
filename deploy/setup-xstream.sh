...
BEGIN
  ...
  ELSE
    -- User exists, reset password to ensure we can connect
    EXECUTE IMMEDIATE 'ALTER USER c##dbzadmin IDENTIFIED BY dbz';
    DBMS_OUTPUT.PUT_LINE('User C##DBZADMIN already exists - password reset');
  END IF;
END;
/
