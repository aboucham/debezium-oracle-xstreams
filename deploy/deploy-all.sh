...
if [ "$STEP_1_STATUS" = "success" ] && [ "$STEP_2_STATUS" = "success" ] && [ "$STEP_3_STATUS" = "success" ] && [ "$STEP_4_STATUS" = "success" ]; then
    ...
    echo ""
    echo "=========================================="
    echo " Quick Verification & Testing"
    echo "=========================================="
    echo ""
    ...
    if [ -n "$KAFKA_POD" ] && [ -n "$ORACLE_POD" ]; then
        echo "3. List Kafka topics (should see oracle-xstreams topics):"
        echo "   oc exec -n ${NAMESPACE} ${KAFKA_POD} -- bin/kafka-topics.sh --bootstrap-server localhost:9092 --list | grep oracle"
        echo ""
        echo "=========================================="
        echo " Test XStreams CDC (Change Data Capture)"
        echo "=========================================="
        echo ""
        echo "Insert test data into Oracle database:"
        echo ""
        echo "oc exec ${ORACLE_POD} -n ${NAMESPACE} -- sqlplus -s c##dbzuser/dbz@ORCLCDB <<'EOF'"
        echo "INSERT INTO CUSTOMERS (id, name, email) VALUES (1001, 'John Doe', 'john@example.com');"
        echo "COMMIT;"
        echo "EXIT;"
        echo "EOF"
        echo ""
        echo "Watch the change event appear in Kafka topic (within 1 second with XStreams!):"
        echo ""
        echo "oc exec -n ${NAMESPACE} ${KAFKA_POD} -- bin/kafka-console-consumer.sh \\"
        echo "  --bootstrap-server localhost:9092 \\"
        echo "  --topic oracle-xstreams.C__DBZUSER.CUSTOMERS \\"
        echo "  --from-beginning \\"
        echo "  --max-messages 1"
        echo ""
        echo "Or watch live changes:"
        echo ""
        echo "oc exec -n ${NAMESPACE} ${KAFKA_POD} -- bin/kafka-console-consumer.sh \\"
        echo "  --bootstrap-server localhost:9092 \\"
        echo "  --topic oracle-xstreams.C__DBZUSER.CUSTOMERS \\"
        ...
        echo "=========================================="
        echo ""
        echo "XStreams delivers 100,000+ events/second with sub-second latency!"
        echo ""
        echo "Insert multiple records and observe near-instant streaming:"
        echo ""
        echo "for i in {2000..2100}; do"
        echo "  oc exec ${ORACLE_POD} -n ${NAMESPACE} -- sqlplus -s c##dbzuser/dbz@ORCLCDB <<EOF"
        echo "INSERT INTO CUSTOMERS (id, name, email) VALUES (\$i, 'User \$i', 'user\$i@example.com');"
        echo "COMMIT;"
        echo "EOF"
        echo "done"
        echo ""
        echo "Watch events streaming in real-time on Kafka Console UI:"
        echo "  https://${CONSOLE_HOST}"
    fi
    echo ""
    echo "=========================================="
